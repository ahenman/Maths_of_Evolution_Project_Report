...
plot(abs(n14(:,1)),abs(n14(:,2)),LineWidth=2.5,Color=C(6,:));
plot(abs(n15(:,1)),abs(n15(:,2)),LineWidth=2.5,Color=C(5,:));
...
text(0,R1,'\boldmath$(0,R_1)$',Interpreter='latex',FontSize=15,VerticalAlignment='bottom', Color= 'white');
text(0,R2,'\boldmath$(0,R_2)$',Interpreter='latex',FontSize=15, Color= 'white');
text(Nbar,Rbar,'\boldmath$(\bar{N},\bar{R})$',Interpreter='latex',FontSize=15, Color= 'white');
text(0,0,'\boldmath$(0,0)$',Interpreter='latex',FontSize=15,VerticalAlignment='middle', Color= 'white');
...
text(4.940275,6,'$\partial V$','Interpreter','latex','VerticalAlignment','bottom','HorizontalAlignment','center','FontSize',20);
...
%Storage for data
Rplot1=zeros(T+1,1);
Rplot2=zeros(T+1,1); %copy of R to show limit cycle
Nplot1=zeros(T+1,1);
Nplot2=zeros(T+1,1); %copy of N to show limit cycle
splot=zeros(T+1,1);
Tplot=zeros(T+1,1);
%initial values
Rplot1(1)=R0;
...
for time=2:T+1
    ...
    %plot max and min values of the last 1000 timesteps to capture cyclic data
    Rplot1(time)=max(n(tend-1000:end,3));
    Rplot2(time)=min(n(tend-1000:end,3));
    if N0==n(end,1) %if resident population not 0, add its data
        Nplot1(time)=max(n(tend-1000:end,1));
        Nplot2(time)=min(n(tend-1000:end,1));
    else %otherwise mutant survived
        Nplot1(time)=max(n(tend-1000:end,2));
        Nplot2(time)=min(n(tend-1000:end,2));
...
%to find the time that s is closest to s* and sbar
...
yline(splot(Text),'r--','LineWidth',1.5,'Label','$s=\bar{s}$', ...
    'Interpreter','latex','FontSize',18,'LabelVerticalAlignment','top');
