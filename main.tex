...
\frontmatter
	...
	\begin{titlepage}
		\begin{center}
			...
			\large
			\begin{align*}
				\textit{by} &\quad \text{Alice Henman} \\
				&\quad \text{Stephenson College}\\[4mm]
				\textit{Supervisor} &\quad \text{Denis Patterson}
			\end{align*}
			\vspace{35mm}
	...
	\end{quote}
	\vfill
	I, \underline{Alice Henman} confirm that I have read, understood, and have adhered to the plagiarism declaration above.
	...
	\begin{center}
		...
		\begin{tabular}{p{4cm} p{9cm} p{2.2cm}}
			...
			\midrule
			Equations throughout the texts that relate to the behaviour of cats in their natural environments  (for instance: \Cref{euler}) & I have modified a \LaTeX ~code produced by AI to add a hidden image of the fabled cat Pusskum to equations that relate to the behaviour of cats in their natural environments (the image would appear when the reader hovers over these equations and holds Command+Shift+C). & CatGPT\\
			\Cref{fig:logistic-growth} & I have used AI to assist me in plotting the pre-, and post-,collision trajectories of 300 red squirrels (predicted by the Red-Squirrel-War model)  & CoPusskum\\
			\bottomrule
		\end{tabular}
	\end{center}
